...
xargs is often used in combination with <code>find</code> in order to do something with the list of files returned by find. Below are some common examples.
...
In order to avoid hitting the system limit, xargs has its own limit to the maximum length of the resulting command and it will automatically truncate the generated command. This limit defaults to 4096 and you can override xargs's limit with the <code>-s</code> flag. This will be particularly important when you are dealing with a large source tree.


h3. Operating on subset of arguments at a time

You might be dealing with commands that can only accept 1 or maybe 2 arguments at a time. For example the diff command operates on two files at a time. The <code>-n</code> flag to xargs specifies how many arguments at a time to supply to the given command. The command will be invoked repeatedly until all input is exhausted. Note that on the last invocation you might get less than the desired number of arguments if there is insufficient input. Since the only simple examples I could think of would be extremely contrived commands, let's simply use xargs to break up the input into 2 arguments per line
